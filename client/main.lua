...
-- Overriding GetWeaponData & RemoveAttachment with new logic
RegisterNUICallback('GetWeaponData', function(cData, cb)
    local data = {
        WeaponData = QBCore.Shared.Items[cData.weapon],
        AttachmentData = FormatWeaponAttachments(cData.ItemData),
        info = cData.ItemData.info
    }
    cb(data)
end)
...
RegisterNUICallback("SetInventoryData", function(data, cb)
    -- (We do skip weight logic for same-inventory in the UI as well,
    -- but the real logic is in the JS. This is just sending the data to server.)
...
RegisterNetEvent("inventory:client:Crafting", function()
    local crafting = {}
    crafting.label = "Crafting"
    crafting.items = GetThresholdItems()
    TriggerServerEvent("inventory:server:OpenInventory", "crafting", math.random(1, 99), crafting)
...
RegisterNetEvent("inventory:client:WeaponAttachmentCrafting", function()
    local crafting = {}
...
RegisterNetEvent("custom-inventory:updateUI", function(inventoryData)
    SendNUIMessage({
        action = "inventory",
        items = inventoryData
    })
...
RegisterNUICallback('DropItem', function(data, cb)
    TriggerServerEvent('inventory:server:DropItem', data)
    cb({})
end)

RegisterNUICallback('SplitItem', function(data, cb)
    TriggerServerEvent('inventory:server:SplitItem', data)
    cb({})
end)

RegisterNetEvent('inventory:client:UpdateSlot', function(slot, itemData)
    local item = nil
